...
export function ChatWindow({ nickname, messages, onSend, compact = false }: { readonly nickname: string; readonly messages: readonly ChatMessage[]; readonly onSend: ChatSendHandler; readonly compact?: boolean }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      const line = LIVE_CHAT_LINES[sequence++ % LIVE_CHAT_LINES.length]
      setActivityMessages((current) => [...current, { ...line, id: `live-chat-${Date.now()}-${sequence}`, time: formatTime() }].slice(-8))
    }, 4_800)
    return () => window.clearInterval(timer)
  }, [compact])
  const allMessages = [...messages, ...activityMessages]
  ...
  return <section className={compact ? `${styles.chatPanel} ${styles.routeChat}` : styles.chatPanel} aria-labelledby={compact ? "route-chat-title" : "global-chat-title"}><header><div><span className={styles.liveDot} /><h2 id={compact ? "route-chat-title" : "global-chat-title"}>전체 채팅</h2></div><span className={styles.onlineCount}><i />12명 접속</span></header><nav className={styles.chatChannels} aria-label="채팅 채널">{channels.map((item) => <button type="button" key={item.id} className={channel === item.id ? styles.channelActive : ""} onClick={() => setChannel(item.id)} aria-pressed={channel === item.id}>{item.label}<small>{item.id === "all" ? allMessages.length : allMessages.filter((messageItem) => messageItem.channel === item.id).length}</small></button>)}</nav><div className={styles.chatMessages} aria-live="polite">{visibleMessages.length ? visibleMessages.slice(compact ? -5 : undefined).map((item) => <p className={item.channel === "system" ? styles.systemMessage : ""} key={item.id}><time>{item.time}</time><strong>{item.author}</strong><span>{item.body}</span></p>) : <p className={styles.emptyChat}>이 채널에는 아직 신호가 없습니다.</p>}</div><form onSubmit={sendMessage}><span className={styles.chatScope}>{channel === "local" ? "지역" : channel === "system" ? "시스템" : "전체"}</span><input value={message} onChange={(event) => setMessage(event.target.value)} maxLength={120} disabled={channel === "system"} placeholder={channel === "system" ? "시스템 채널은 읽기 전용입니다" : `${nickname}의 메시지 입력`} aria-label="전체 채팅 메시지" /><span className={styles.chatLimit}>{message.length}/120</span><button type="submit" disabled={channel === "system" || !message.trim()} aria-label="채팅 보내기">전송</button></form></section>
}
...
function ChatDrawer({ nickname, messages, onSend }: { readonly nickname: string; readonly messages: readonly ChatMessage[]; readonly onSend: (body: string, channel: Exclude<ChatChannel, "system">) => void }) {
  ...
  return <div className={`${styles.chatDrawer} ${open ? styles.chatDrawerOpen : styles.chatDrawerClosed}`}><div className={styles.chatDrawerPanel} id="route-chat-panel"><ChatWindow compact nickname={nickname} messages={messages} onSend={onSend} /></div><button className={styles.chatDrawerToggle} type="button" onClick={() => setOpen((current) => !current)} aria-expanded={open} aria-controls="route-chat-panel"><span className={styles.liveDot} />{open ? "채팅 닫기" : "전체 채팅"}<small>12명 접속</small><b>{open ? "⌃" : "⌄"}</b></button></div>
}
...
function RouteMap({ progress, nickname, messages, onSend, onEnterTerra }: { readonly progress: number; readonly nickname: string; readonly messages: readonly ChatMessage[]; readonly onSend: (body: string, channel: Exclude<ChatChannel, "system">) => void; readonly onEnterTerra: () => void }) {
  ...
  return <section className={styles.routeView} aria-labelledby="route-title"><div className={styles.routeMap}><div className={`${styles.routeAsset} ${styles.routeSpace}`} aria-hidden="true" /><div className={`${styles.routeAsset} ${styles.routeTerra}`} aria-hidden="true" /><div className={`${styles.routeAsset} ${styles.routeLuna}`} aria-hidden="true" /><div className={`${styles.routeAsset} ${styles.routeSol}`} aria-hidden="true" /><div className={styles.routeIntro}><span>01 / STELLAR ROUTE</span><h1 id="route-title">{english ? <>Choose the<br /><em>next route.</em></> : <>다음 항로를<br /><em>선택하세요.</em></>}</h1><p>{english ? "TERRA is open now. Other routes unlock through shared contributions." : "TERRA는 지금 입장할 수 있습니다. 다른 항로는 모든 탐사대의 공동 기여로 해금됩니다."}</p></div>{(["terra", "luna", "sol"] as const).map((id) => <button className={`${styles.planet} ${styles[`planet${id[0].toUpperCase()}${id.slice(1)}`]} ${selectedPlanet === id ? styles.planetActive : ""}`} type="button" key={id} onClick={() => setSelectedPlanet(id)} aria-pressed={selectedPlanet === id} aria-expanded={selectedPlanet === id} aria-controls="planet-information"><span className={styles.planetCore} /><strong>{PLANETS[id].name}</strong><small>{id === "terra" ? "입장 가능" : id === "luna" ? `복구 ${progress}%` : "잠김"}</small></button>)}<div className={styles.mapBrand} aria-label="FIRST LIGHT"><strong>FIRST LIGHT</strong><span><i />✦</span><small>TERRA</small></div><ChatDrawer nickname={nickname} messages={messages} onSend={onSend} /><span className={styles.mapCoordinate}>34° 18′ 22″ N&nbsp;&nbsp; / &nbsp;&nbsp;FIRST LIGHT NETWORK</span></div>{selectedPlanet && planet ? <aside id="planet-information" className={styles.planetPanel} aria-live="polite"><button className={styles.panelClose} type="button" onClick={() => setSelectedPlanet(null)} aria-label="행성 정보 닫기">×</button><div><span className={styles.panelNumber}>0{selectedPlanet === "terra" ? 1 : selectedPlanet === "luna" ? 2 : 3}</span><span className={styles.status}>{planet.status}</span></div><span className={styles.kicker}>{planet.kicker}</span><h2>{planet.name}</h2><p>{planet.summary}</p>{selectedPlanet === "luna" ? <div className={styles.unlockProgress}><span><strong>공동 항로 복구</strong><b>{progress}%</b></span><i><b style={{ width: `${progress}%` }} /></i><small>목재 · 철 · 시그널 잔해 · 재활용 부품</small></div> : null}<div className={styles.panelDivider} />{selectedPlanet === "terra" ? <button className={styles.enterButton} type="button" onClick={onEnterTerra}>{LEGACY_TERRA_ENTRY_LABEL}<span>→</span></button> : <button className={styles.lockedButton} type="button" disabled>현재 이동할 수 없습니다</button>}<small className={styles.panelHint}>다른 행성은 공동 복구율에 따라 열립니다.</small></aside> : null}</section>
}
...
export function PlazaNewsPanel({ headingId = "plaza-news-title", className }: { readonly headingId?: string; readonly className?: string }) {
  const { language } = useLanguage()
  const [news, setNews] = useState<readonly PlazaNewsItem[]>(() => PLAZA_NEWS_LINES.slice(0, 3).map((item, index) => ({ ...item, id: `seed-news-${index}`, time: index === 0 ? "방금" : `${index * 2}분 전` })))
  useEffect(() => {
    let sequence = 3
    const timer = window.setInterval(() => {
      const item = PLAZA_NEWS_LINES[sequence++ % PLAZA_NEWS_LINES.length]
      setNews((current) => [{ ...item, id: `live-news-${Date.now()}-${sequence}`, time: "방금" }, ...current].slice(0, 5))
    }, 5_600)
    return () => window.clearInterval(timer)
  }, [])
  return <section className={className ? `${styles.plazaNews} ${className}` : styles.plazaNews} aria-labelledby={headingId}>
    <header><div><span className={styles.liveDot} /><h2 id={headingId}>{language === "en" ? "PLAZA NEWS" : "광장 뉴스"}</h2></div><small>LIVE</small></header>
    <ul aria-live="polite">{news.map((item) => <li key={item.id}><span>{item.category}</span><p>{item.body}</p><time>{item.time}</time></li>)}</ul>
  </section>
}
...
function PlazaView({ nickname, messages, onSend }: { readonly nickname: string; readonly messages: readonly ChatMessage[]; readonly onSend: (body: string, channel: Exclude<ChatChannel, "system">) => void }) {
  ...
  const phase = PLAZA_PHASES[phaseId]
  ...
  return <section className={`${styles.contentView} ${styles.plazaView}`} aria-label="광장">
      ...
      <div className={styles.plazaInteractionLayer} aria-label="광장 신호 상호작용">
        {activeSignals.map((signal) => { const signalTitle = signal.kind === "timing" ? "신호 타이밍 정렬" : "신호 패턴 해독"; return <button key={signal.id} className={`${styles.plazaInteraction} ${signal.kind === "timing" ? styles.plazaTimingSpot : styles.plazaPatternSpot}`} style={{ left: `${signal.left}%`, top: `${signal.top}%` }} type="button" onClick={() => setActiveGame(signal.kind)} aria-haspopup="dialog"><span>미확인 신호</span><strong>{signalTitle}</strong><small>{phase.label} 활성</small></button> })}
      </div>
      ...
      <section className={styles.plazaTimeCard} aria-labelledby="plaza-time-title">
        <header><strong id="plaza-time-title">TERRA</strong><span>{testPhase ? "테스트" : "실시간"}</span></header>
        <div className={styles.plazaClock}><strong>{clock}</strong><span>· {phase.label}</span></div>
        <p>시간대에 따라 바뀌는 광장 배경</p>
        <div className={styles.plazaPhaseMeta}><span>{phase.time}</span><span>{phase.source} · {phase.treatment}</span></div>
        <div className={styles.plazaTestControls} role="group" aria-label="시간대 테스트">
          <button type="button" className={testPhase === null ? styles.plazaPhaseActive : ""} onClick={() => setTestPhase(null)} aria-pressed={testPhase === null}>실시간</button>
          {(Object.keys(PLAZA_PHASES) as PlazaPhase[]).map((item) => <button type="button" key={item} className={testPhase === item ? styles.plazaPhaseActive : ""} onClick={() => setTestPhase(item)} aria-pressed={testPhase === item}>{PLAZA_PHASES[item].label}</button>)}
        </div>
      </section>
      <PlazaNewsPanel />
...
export function TerraCommandCenter({ profile, inventory, progress, resourceCount, messages, onSendMessage, onEnterTerra, musicEnabled, onToggleMusic, onTabChange }: TerraCommandCenterProps) {
  ...
  useEffect(() => {
    const expirationTimers = new Set<number>()
    let sequence = 0
    const spawnMission = () => {
      const kind: MissionKind = Math.random() < 0.5 ? "real" : "ingame"
      const candidates = MISSION_POOL.filter((mission) => mission.kind === kind)
      const template = candidates[Math.floor(Math.random() * candidates.length)]
      if (!template) return
      const mission = { ...template, id: `live-${Date.now()}-${sequence++}` }
      setMissions((current) => [...current, mission])
      const lifetime = Math.random() < 0.5 ? 7000 : 10000
      let expirationTimer = 0
      expirationTimer = window.setTimeout(() => {
        expirationTimers.delete(expirationTimer)
        setMissions((current) => current.filter((item) => item.id !== mission.id || item.status !== "not-started"))
      }, lifetime)
      expirationTimers.add(expirationTimer)
    }
    const interval = window.setInterval(spawnMission, 5000)
    return () => {
      window.clearInterval(interval)
      expirationTimers.forEach((timer) => window.clearTimeout(timer))
    }
  }, [])
