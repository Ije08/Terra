...
import { getCharacterAtlasPath, getCharacterInteractionSheetPath, getCharacterWalkSheetPath, getDirectionAtlasCell, type SpriteDirection } from "../game/characters"
...
function PaperDollSprite({ character, position, direction, frame, interactionFrame, interacting }: { readonly character: CharacterProfile; readonly position: Point; readonly direction: SpriteDirection; readonly frame: number; readonly interactionFrame: number; readonly interacting: boolean }) {
  ...
  const usesGeneratedSheets = walkSheetPath !== null && interactionSheetPath !== null
  const action = interacting && usesGeneratedSheets ? "interaction" : "walk"
  const sheetPath = action === "interaction" ? interactionSheetPath : walkSheetPath
  const cardinalDirection = toCardinalSpriteDirection(direction)
  const sheetRow = { down: 0, left: 1, right: 2, up: 3 }[cardinalDirection]
  const sheetFrame = action === "interaction" ? interactionFrame : frame
  ...
  const spriteStyle: CSSProperties = {
    ...
    backgroundPosition: usesGeneratedSheets ? `${(sheetFrame / 3) * 100}% ${(sheetRow / 3) * 100}%` : `${cell.column * 50}% ${cell.row * 50}%`,
    backgroundSize: usesGeneratedSheets ? "400% 400%" : "300% 300%",
  }
  return <div className={`player-sprite-anchor${interacting ? " is-interacting" : ""}`} data-sprite-action={action} data-sprite-direction={direction} data-sprite-frame={sheetFrame} style={{ left: `${(position.x / WORLD_SIZE.x) * 100}%`, top: `${(position.y / WORLD_SIZE.y) * 100}%` }} aria-hidden="true">
...
export function TerraCanvas({ character, scene, onCollectResource, onLeaveHome }: TerraCanvasProps) {
  ...
  const animationStateRef = useRef<{ direction: SpriteDirection; frame: number }>({ direction: "down", frame: 0 })
  ...
  useEffect(() => {
    ...
    animationStateRef.current = { direction: "down", frame: 0 }
  ...
  useEffect(() => {
    ...
    const tick = (time: number) => {
      ...
      if (length === 0 || reducedMotionRef.current) {
        animationClockRef.current = 0
        ...
        if (animationClockRef.current >= 0.12) {
          const steps = Math.floor(animationClockRef.current / 0.12)
          animationClockRef.current %= 0.12
  ...
  return <div className="terra-canvas-wrap">
    ...
    <PaperDollSprite character={character} position={position} direction={spriteDirection} frame={spriteFrame} interactionFrame={interactionFrame} interacting={interacting} />
